...
- **R16 — Finish R14 for the *dynamic* console builders.** `effort: M–L` — 🟡 IN PROGRESS (v1.14.0–1.15.0). Dynamic-builder lock engine shipped (`renderDynamicPrompt` + `canonicalCoverage` in `gen-html-prompts.mjs`); §T1 (`buildTier1Text` ← /broad-scan) and §T2a (`buildTier2AuditText` ← /targeted-audit) reconciled to 100% and **locked** by `--assert` (INV-36). 2 of 4 dynamic builders locked.
